...
- 접속 기기명과 고정 IP 등록명을 별도로 수집하고 MAC 기준으로 병합
...
고정 DHCP 목록은 해당 메뉴를 제공하는 펌웨어에서만 표시됩니다. Beta/모바일
UI에서도 구형 DHCP 페이지가 함께 제공되면 이름과 유선 목록을 보완합니다.
기기 선택 화면과 `device_tracker` 속성에서는 기기가 보고한 `device_name`과
공유기에 등록한 `reservation_name`을 구분합니다. 표시 이름은 등록명을 우선합니다.
...
| 탐지 주기 | 공유기에서 접속 기기 목록을 다시 조회하는 주기(10~3600초) | `30` |
...
- 이 통합은 ipTIME의 공개 외부 API가 아니라 관리자 UI의 내부 JSON/HTML 형식을
  읽습니다. 펌웨어 업데이트로 형식이 바뀌면 이름 등 보조 정보가 일시적으로
  누락될 수 있습니다. 디버그 로그에는 응답 원문이나 이름을 남기지 않고 필드명과
  테이블 헤더만 남겨 새 형식에 대응할 수 있게 합니다.
